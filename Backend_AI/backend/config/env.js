...
const schema = Joi.object({
  ...
  // Required for invite activation, OTP delivery, and email verification.
  // Set both as GitHub secrets:
  //   AZURE_COMMUNICATION_CONNECTION_STRING  ← from Azure portal → ACS resource → Keys
  //   AZURE_SENDER_EMAIL                     ← verified sender address in ACS
  // If neither is set, email features are disabled and sendEmail() throws
  // EMAIL_NOT_CONFIGURED (callers must handle this gracefully).
  AZURE_COMMUNICATION_CONNECTION_STRING: Joi.string().trim().optional().allow(""),
  AZURE_SENDER_EMAIL: Joi.string().email().optional().allow(""),

  // Legacy nodemailer (kept for local dev fallback — not used in production)
...
function parseOrigins(value) {
  const base = [value.FRONTEND_URL, value.ADMIN_FRONTEND_URL].filter(Boolean)
  const fromList = String(value.CORS_ALLOWED_ORIGINS || "")
    .split(",")
    .map((x) => x.trim())
    .filter(Boolean)
...
function getEnv() {
  ...
  _env = Object.freeze({
    ...
    platformAdminEmail: value.PLATFORM_ADMIN_EMAIL.toLowerCase().trim(),
    ...
    azureCommunicationConnectionString: value.AZURE_COMMUNICATION_CONNECTION_STRING || null,
    azureSenderEmail: value.AZURE_SENDER_EMAIL || null,

    // Legacy nodemailer (local dev only)
    ...
    googleCallbackUrl: value.GOOGLE_CALLBACK_URL || null,
...
// Proxy so all existing code using `env.X` continues to work unchanged
const env = new Proxy({}, {
  get(_, prop) {
    return getEnv()[prop]
  }
