...
const { hardDeleteCollege } = require("../services/collegeCleanupService")
...
const normalizeStudentLimit = (value) => {
  if (value === undefined) return undefined
  if (value === null || value === "" || Number(value) === 0) return null
  const parsed = Number(value)
  if (!Number.isInteger(parsed) || parsed < 1) {
    const error = new Error("studentLimit must be a positive integer or 0 for unlimited")
    error.statusCode = 400
    error.code = "INVALID_STUDENT_LIMIT"
    throw error
  }
  return parsed
}
...
exports.listColleges = async (req, res) => {
  try {
    ...
    const data = colleges.map((college) => {
      ...
      const studentCount = Number(studentMap[collegeId] || 0)
      ...
      return {
        ...
        studentCount,
        studentLimit,
        remainingStudentSlots: studentLimit === null
          ? null
...
exports.createCollege = async (req, res) => {
  try {
    ...
    const studentLimit = normalizeStudentLimit(req.body?.studentLimit)

    // Case-insensitive duplicate check across ALL colleges (including inactive)
    ...
    const college = await College.create({
      ...
      departmentLimit: departmentLimit === undefined ? (env.defaultCollegeDepartmentLimit ?? null) : departmentLimit,
      studentLimit: studentLimit === undefined ? null : studentLimit,
    })
...
exports.updateCollege = async (req, res) => {
  try {
    ...
    if (req.body?.name !== undefined) {
      ...
      // Check for duplicate name (excluding the current college)
    ...
    if (departmentLimit !== undefined) {
      ...
      if (departmentLimit !== null && capacity.departmentCount > departmentLimit) {
        return res.status(409).json({
          message: "Department limit cannot be below the current department count",
    ...
    if (studentLimit !== undefined) {
      const studentCount = await User.countDocuments({ collegeId: id, role: ROLES.STUDENT })
      if (studentLimit !== null && studentCount > studentLimit) {
        return res.status(409).json({
          message: "Student limit cannot be below the current student count",
          studentCount,
        })
      }
      updates.studentLimit = studentLimit
    }
    ...
    const studentCount = await User.countDocuments({ collegeId: college._id, role: ROLES.STUDENT })
    const sl = college.studentLimit ?? null
    ...
    return res.status(200).json({
      ...
      studentLimit: sl,
      studentCount,
      remainingStudentSlots: sl === null ? null : Math.max(sl - studentCount, 0),
      updatedAt: college.updatedAt,
    })
  } catch (error) {
...
exports.deleteCollege = async (req, res) => {
  try {
    ...
    // Hard-delete: permanently removes the college and all related data
    const summary = await hardDeleteCollege(id, { actor: req.user })

    await logAudit(req.user, "DELETE_COLLEGE", "College", id, req, {
      collegeName: summary.collegeName,
      deletedCounts: summary,
    })

    return res.status(200).json({
      message: `College "${summary.collegeName}" and all its data have been permanently deleted.`,
      deletedCounts: {
        users: summary.users,
        departments: summary.departments,
        invitations: summary.invitations,
        placementDrives: summary.placementDrives,
        driveCandidates: summary.driveCandidates,
        companies: summary.companies,
        resumes: summary.resumes,
        academicProfiles: summary.academicProfiles,
        placementNotifications: summary.placementNotifications,
        reportApprovals: summary.reportApprovals,
        companyShortlists: summary.companyShortlists,
        downloadUsage: summary.downloadUsage,
    ...
    return res.status(error.statusCode || 500).json({ message: error.message || "Failed to delete college" })
  }
}
