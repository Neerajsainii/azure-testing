...
function issueAccessToken(account, role) {
  ...
  return { accessToken, csrfToken, expiresAt, tokenId }
}
  ...
  try {
    ...
    if (env.emailVerificationRequired && !user.isEmailVerified) {
      ...
      try {
        ...
        logger.error("signup_email_failed", { error: emailError.message })
        emailSent = false
      }
...
exports.requestEmailVerification = async (req, res, next) => {
  try {
    ...
    try {
      ...
      logger.error("verification_email_failed", { error: emailError.message })
    }
...
exports.getSession = async (req, res, next) => {
  try {
    ...
    // Use structured logger — never raw console.log which bypasses redaction
    logger.info("session_requested", {
...
exports.logout = async (req, res, next) => {
  try {
    // Revoke the token so authMiddleware's jti check blocks it immediately,
    // even though it hasn't expired yet.  Without this, a logged-out token
    // stays valid for the full jwtExpiresIn window.
    ...
    if (jti) {
      ...
      await RevokedToken.create({
        jti,
        expiresAt: exp,
        userId: req.user?._id || null,  // optional — never used for lookups
      }).catch((err) => {
        // Non-fatal — log but don't block the logout response.
        // The token will naturally expire on its own.
        logger.warn("logout_revoke_failed", {
          requestId: req.requestId || null,
          message: err.message,
        })
      })
...
exports.googleCallback = async (req, res, next) => {
  try {
    ...
    // Google OAuth is a browser redirect flow — the browser is sitting on the
    // callback URL after being redirected from Google.  We must redirect back
    // to the frontend with the token; returning JSON would show raw JSON to
    // the user instead of loading the app.
    //
    // The frontend's AuthContext should read these params on the callback page
    // and store the token (e.g. /auth/callback?token=...&csrfToken=...&role=...).
    const frontendCallbackUrl = new URL(
      "/auth/callback",
      env.frontendUrl
    )
    frontendCallbackUrl.searchParams.set("token", accessToken)
    frontendCallbackUrl.searchParams.set("csrfToken", csrfToken)
    frontendCallbackUrl.searchParams.set("expiresAt", String(expiresAt))
    frontendCallbackUrl.searchParams.set("role", ROLES.PLATFORM_ADMIN)

    return res.redirect(frontendCallbackUrl.toString())
  } catch (error) {
    // Redirect to error page rather than showing a raw 500
    const errorUrl = new URL("/login?error=google_failed", env.frontendUrl)
