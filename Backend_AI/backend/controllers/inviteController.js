...
const { assertStudentCapacity } = require("../services/studentCapacityService")
...
exports.inviteStudent = async (req, res) => {
  try {
    ...
    // Enforce student capacity limit
    try {
      await assertStudentCapacity(req.user.collegeId)
    } catch (capacityError) {
      return res.status(capacityError.statusCode || 409).json({
        success: false,
        message: "Student limit reached for this college. Contact your admin to increase the limit.",
