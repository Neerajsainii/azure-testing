...
import type { AdministrationRole } from "@/lib/loginFlow"
import { toSafeAuthErrorMessage } from "@/lib/authErrors"
import { Eye, EyeOff } from "lucide-react"

const ROLE_OPTIONS: Array<{ label: string; value: AdministrationRole }> = [
...
export default function AdministrationLogin() {
  ...
  const [selectedRole, setSelectedRole] = useState<AdministrationRole | "">("")
    ...
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0b0424] via-[#1a0f4a] to-[#2b1d77] px-4 py-10">
      <div className="w-full max-w-md space-y-4">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-white">
          <h1 className="text-lg font-semibold">Administration Login</h1>
          <p className="mt-1 text-sm text-white/70">Select your role before signing in.</p>

          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div>
              <label className="block text-sm font-medium text-white/80" htmlFor="admin-role">
                Role
              </label>
              <select
                id="admin-role"
                value={selectedRole}
                onChange={(e) => setSelectedRole(e.target.value as AdministrationRole | "")}
          ...
          <p className="mt-4 text-center text-sm text-white/60">
            <Link to="/register" className="text-white/90 underline hover:no-underline">
